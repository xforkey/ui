...
function ThemeToggleButton({
    ...
    return (
        <RadioGroupPrimitive.Item
            ...
            className={cn(
                "text-primary aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive aspect-square size-6 shrink-0 rounded-full shadow-xs transition-[color,box-shadow] outline-none disabled:cursor-not-allowed disabled:opacity-50",
                "data-[state=checked]:bg-white data-[state=checked]:ring data-[state=checked]:inset-ring data-[state=checked]:ring-gray-950/10 data-[state=checked]:inset-ring-white/10 sm:p-0 dark:data-[state=checked]:bg-gray-700 dark:data-[state=checked]:text-white dark:data-[state=checked]:ring-transparent",
                className
            )}
            {...props}
        >
...
export default function ThemeToggle() {
    ...
    return (
        <RadioGroup
            value={theme ?? "system"}
            onValueChange={(value) => onChange(value as Theme, setTheme)}
            className="relative z-0 inline-grid grid-cols-3 gap-0.5 bg-gray-950/5 p-0.75 text-gray-950 dark:bg-white/10 rounded-full"
        >
